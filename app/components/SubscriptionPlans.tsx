...
export default function SubscriptionPlans() {
  ...
  const plans = [
    ...
    {
      name: "Pro",
      description: "Dla rosnących ośrodków posiadających kilka unikalnych obiektów.",
      monthlyPrice: 79,
      yearlyPrice: 63,
      features: [
  ...
  return (
    <section id="pricing" className="py-24 px-6 bg-background transition-colors border-t border-border-custom relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="mt-4 text-sm text-fg-custom/60 font-light leading-relaxed">
            Oferuj pobyty butikowe w profesjonalnej oprawie wizualnej i technologicznej. Nasze witryny konwertują odwiedzających na rezerwacje od pierwszego dnia.
          </p>

          {/* Przełącznik Miesięczny / Roczny */}
          <div className="flex items-center justify-center gap-3 mt-10">
            <span className={`text-xs font-semibold ${!isYearly ? "text-fg-custom" : "text-fg-custom/40"}`}>
          ...
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  {/* Nazwa i opis */}
                  <h3 className="text-xl font-bold text-fg-custom">{plan.name}</h3>
                  ...
                  <div className="mt-6 pb-6 border-b border-border-custom flex items-baseline">
                    ...
                    <span className="text-xs text-fg-custom/60 ml-2 font-medium">/ miesiąc</span>
                  </div>

                  {/* Cechy */}
                  <ul className="mt-8 space-y-4">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-3 text-xs text-fg-custom/80 font-light">
        ...
        <div className="text-center mt-12 flex justify-center items-center gap-2 text-xs text-fg-custom/40 font-medium">
          ...
          <span>Masz pytania odnośnie wdrożenia? Skonsultuj się z naszym zespołem technicznym.</span>
        </div>
      </div>
    </section>
  );
