...
class MockAuth {
  ...
  async signInWithEmailAndPassword(email: string) {
    ...
    const mockUser: MockUser = { uid: "mock-user-123", email, displayName: "Gość Wild Haven" };
    this.user = mockUser;
    if (typeof window !== "undefined") {
  ...
  async createUserWithEmailAndPassword(email: string) {
    const mockUser: MockUser = { uid: "mock-user-123", email, displayName: "Nowy Właściciel" };
    this.user = mockUser;
    if (typeof window !== "undefined") {
      localStorage.setItem("mock_user", JSON.stringify(mockUser));
    }
...
class MockFirestore {
  ...
  async addDoc(collectionName: string, data: Record<string, unknown>) {
    ...
    const newDoc = { id: `doc_${Date.now()}`, ...data, createdAt: new Date().toISOString() };
