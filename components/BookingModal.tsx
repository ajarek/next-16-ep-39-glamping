...
const ADDONS = [
  {
    id: "kayak",
    name: "Dwuosobowy kajak",
    price: 75,
    description: "Dostęp do kajaka na czas całego pobytu (PLN 75/dzień)",
    perDay: true,
  },
  {
    id: "food",
    name: "Lokalne wyżywienie",
    price: 131,
    description:
      "Kosz delikatesowy z lokalnymi produktami dostarczany rano (PLN 131/dzień/osobę)",
    perPerson: true,
    perDay: true,
  },
  {
    id: "sauna",
    name: "Dostęp do sauny leśnej",
    price: 188,
    description:
      "Jednorazowy nielimitowany seans w saunie opalanej drewnem (PLN 188/pobyt)",
    perDay: false,
  },
  {
    id: "stargazing",
    name: "Zestaw do obserwacji gwiazd",
    price: 56,
    description:
      "Profesjonalna lornetka i teleskop astronomiczny z mapą nieba (PLN 56/pobyt)",
    perDay: false,
  },
]
...
export default function BookingModal({
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (startDate && endDate) {
        ...
        const end = new Date(endDate)
        if (end <= start)
          stepErrors.endDate = "Data zakończenia musi być po dacie rozpoczęcia"
        if (start < new Date(new Date().setHours(0, 0, 0, 0)))
          stepErrors.startDate = "Data nie może być z przeszłości"
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({
        general: "Wystąpił nieoczekiwany błąd sieci. Spróbuj ponownie.",
      })
    } finally {
      setLoading(false)
    }
                ...
                <div className='flex items-center gap-4 w-full'>
                  {[1, 2, 3, 4].map((s) => (
                    ...
                    >
                      ...
                      <span
                        className={`hidden sm:inline ${step === s ? "text-fg-custom font-bold" : ""}`}
                      >
                        {s === 1 && "Termin"}
                        {s === 2 && "Dane gościa"}
                        {s === 3 && "Dodatki"}
                      ...
                      {s < 4 && (
                        <div className='h-px bg-border-custom flex-1 hidden sm:block' />
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className='p-8 overflow-y-auto flex-1'>
              ...
              {success ? (
                ...
                >
                  ...
                  <h3 className='text-2xl font-light text-fg-custom'>
                    Rezerwacja potwierdzona!
                  </h3>
                  <p className='mt-2 text-sm text-fg-custom/60 max-w-sm mx-auto font-light'>
                    Twój pobyt w **{selectedLocation?.name}** został pomyślnie
                    zarezerwowany. Szczegóły wysłaliśmy na podany adres e-mail.
                  </p>
                      ...
                      <div>
                        ...
                        <div className='flex items-center gap-3'>
                          <button
                            type='button'
                            onClick={() =>
                              setGuestsCount((g) => Math.max(1, g - 1))
                            }
                          ...
                          <button
                            type='button'
                            onClick={() =>
                              setGuestsCount((g) => Math.min(10, g + 1))
                            }
                            className='w-10 h-10 border border-border-custom rounded-lg bg-card-custom flex items-center justify-center font-bold text-fg-custom hover:bg-brand-muted/10 focus:outline-none cursor-pointer'
                          >
                            +
                  ...
                  {step === 3 && (
                    ...
                    >
                      ...
                      <p className='text-xs text-fg-custom/60 font-light'>
                        Wzbogać swój wypoczynek o unikalne usługi, które
                        przygotujemy dla Ciebie na miejscu.
                      </p>
                    ...
                    >
                      ...
                      <div className='p-6 rounded-2xl border border-border-custom bg-brand-muted/5 space-y-4 text-sm text-fg-custom'>
                        <div className='flex justify-between border-b border-border-custom/50 pb-3'>
                          <span className='font-light text-fg-custom/60'>
                            Obiekt:
                          </span>
                        ...
                        {selectedAddons.length > 0 && (
                          <div className='border-b border-border-custom/50 pb-3 space-y-1'>
                            <span className='font-light text-fg-custom/60 text-xs block mb-1'>
                              Wybrane dodatki:
                            </span>
                            {selectedAddons.map((addonId) => {
                              ...
                              return (
                                ...
                                >
                                  <span className='text-fg-custom/80 font-medium'>
                                    • {addon?.name}
                                  </span>
                                  <span className='font-bold text-fg-custom/60'>
                                    +PLN {addon?.price}
                                  </span>
                                </div>
                              )
                        ...
                        <div className='pt-2 flex justify-between items-end'>
                          <div>
                            <span className='text-xs text-fg-custom/60 block font-light'>
                              Łączny koszt:
                            </span>
                            <span className='text-xs font-semibold text-fg-custom/40'>
                              (baza: PLN {basePrice} + dodatki: PLN{" "}
                              {addonsPrice})
                            </span>
                          </div>
                      ...
                      <p className='text-[10px] text-fg-custom/40 text-center font-light leading-relaxed max-w-sm mx-auto'>
                        Klikając przycisk potwierdzenia rezerwacji akceptujesz
                        Regulamin portalu Wild Haven oraz politykę ochrony
                        danych osobowych.
                      </p>
                    </motion.div>
                  )}
